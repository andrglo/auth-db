...
describe('Users', function() {
  ...
  it('Email andre@example.com cannot be updated without sending the username', function(done) {
    authDb.email
        .update({sameDate: '2016-02-19'}, 'andre@example.com')
        .then(function(res) {
  ...
  it('Email andre@example3.com can be removed', function(done) {
    authDb.email
        .remove('andre@example3.com', 'andre')
        .then(function(res) {
          expect(res).equal(true)
          done()
        })
    ...
    authDb.users
        ...
        .catch(function(err) {
          err.message.should.equal(
              'To update/create/delete a email use email api'
          )
          done()
        })
        .catch(done)
...
describe('Roles', function() {
  ...
  it('Marketing should have permission to spec any', function(done) {
    authDb.roles
        .hasPermission('mArketing', 'spec', 'any')
  ...
  it('Marketing should not have permission to habilis/cadastro get', function(done) {
    authDb.roles
        .hasPermission('mArketing', 'habilis/cadastro', 'get')
        .then(function(allowed) {
          allowed.should.equal(false)
          done()
        })
  ...
  it('nor to habilis/cadastro', function(done) {
    authDb.roles
        .hasPermission('mArketing', 'habilis/cadastro')
        .then(function(allowed) {
          allowed.should.equal(false)
          done()
        })
  ...
  it('but do to habilis/cadastro post', function(done) {
    authDb.roles
        .hasPermission('mArketing', 'habilis/cadastro', 'post')
        .then(function(allowed) {
          allowed.should.equal(true)
          done()
        })
  ...
  it('Role mArketing in a array should have permission to habilis/cadastro post - last position', function(done) {
    authDb.roles
        .hasPermission(['a', 'b', 'c', 'mArketing'], 'habilis/cadastro', 'post')
        .then(function(allowed) {
          allowed.should.equal(true)
          done()
        })
  ...
  it('Role mArketing if not in the array should should not have permission to habilis/cadastro post', function(done) {
    authDb.roles
        .hasPermission(['a', 'b', 'c'], 'habilis/cadastro', 'post')
        .then(function(allowed) {
          allowed.should.equal(false)
          done()
        })
  ...
  it('Role undefined should be rejected', function(done) {
    authDb.roles
        .hasPermission([void 0], 'habilis/cadastro', 'post')
        .then(function(allowed) {
          allowed.should.equal(false)
          done()
        })
  ...
  it('should test 1000 times and have access permission', function(done) {
    ...
    for (let i = 0; i < 1000; i++) {
      promises = promises.then(function() {
        return authDb.roles
            .hasPermission('teste de benchmark', 'resource')
            .then(function(allowed) {
              allowed.should.equal(true)
            })
      })
...
describe('Users removal', function() {
  it('should reject due to user is active', async () => {
    try {
      await authDb.users.remove('john')
      throw new Error('Invalid user removal')
    } catch (err) {
      err.code.should.equal('activeUser')
    }
  })
  it('should remove inactive user', async () => {
    let data = await redis.hgetall('auth-db:emails:andre@example.com')
    expect(data.username).equal('andre')
    data = await redis.hgetall('auth-db:emails:andre@example2.com')
    expect(data.username).equal('andre')
    await authDb.users.remove('andre')
    data = await redis.hgetall('auth-db:emails:andre@example.com')
    expect(data).eql({})
    data = await redis.hgetall('auth-db:emails:andre@example2.com')
    expect(data).eql({})
  })
